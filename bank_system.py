...
class BankSystem:
    ...
    def register(self, account_no, name, password, mobile):
        ...
        if len(password) != 6 or not password.isdigit():
            return False, "Password must be a 6-digit number."
        if account_no in self.users:
            return False, "Account number already registered."
        for user in self.users.values():
            if user["name"] == name:
    ...
    def login(self, identifier, password):
        for acc_no, user in self.users.items():
            if (identifier == acc_no or identifier == user["name"] or identifier == user["mobile"]) and str(user["password"]) == str(password):
                return acc_no
        return None

    def forgot_password(self, mobile_or_name):
        for user in self.users.values():
            if user["mobile"] == mobile_or_name or user["name"] == mobile_or_name.upper():
    ...
    def deposit(self, acc_no, amount):
        self.users[acc_no]["balance"] += amount
        self.users[acc_no]["history"].append(f"{datetime.now()} - Deposited ₹{amount}")
        self.save()

    def withdraw(self, acc_no, amount):
        if self.users[acc_no]["balance"] >= amount:
            self.users[acc_no]["balance"] -= amount
            self.users[acc_no]["history"].append(f"{datetime.now()} - Withdrew ₹{amount}")
            self.save()
            return True
        return False

    def transfer(self, sender_acc, recipient_info, amount):
        recipient_acc = None
        for acc, user in self.users.items():
            if user["name"] == recipient_info.upper() or user["mobile"] == recipient_info:
                recipient_acc = acc
                break
        ...
        if recipient_acc and self.users[sender_acc]["balance"] >= amount:
            ...
            self.users[sender_acc]["balance"] -= amount
            self.users[recipient_acc]["balance"] += amount
            self.users[sender_acc]["history"].append(f"{datetime.now()} - Transferred ₹{amount} to {recipient_info.upper()}")
            self.users[recipient_acc]["history"].append(f"{datetime.now()} - Received ₹{amount} from {sender_name}")
            self.save()
            return True
        return False
